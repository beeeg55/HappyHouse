...
<script>
	...
	function callHouseDealInfo(aptName, dong) {
		$.get("${pageContext.request.contextPath}/search/detail"
				,{name:aptName, dong:dong}
			...
			<div class="col-sm-9">
				...
				$(document).ready(function(){
					...
					$("#dong").change(function() {
						$.get("${pageContext.request.contextPath}/search/apt"
								,{dong:$("#dong").val()}
								,function(data, status){
									$("#searchResult").empty();
									$.each(data, function(index, vo) {
				...
				function searchByName() {
					$.get("${pageContext.request.contextPath}/search/aptName"
							,{aptName:$("#aptName").val()}
							,function(data, status){
								$("#searchResult").empty();
								$.each(data, function(index, vo) {
				...
				function geocode(jsonData) {
					...
					<input type="text" placeholder="아파트이름" id="aptName" name="aptName"
						class="form-control"> <input type="button" id="aptNameBtn"
						class="form-control" value="검색" onclick="searchByName()">
